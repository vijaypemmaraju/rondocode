...
p('poly', note('{c3 e3 g3, c4 b3}%4').sound('pluck').gain(0.5))`,
      ...
      p("The same is true of a COMBINATOR's count, not just a mini-notation atom's: `fast <1 2>` runs a part at one speed then double, `ply <2 3>` ratchets differently each cycle, `euclid <3 5> 8` walks between figures, and even the period of a scheduled transform patterns, so `every <2 4>: rev` reverses on a moving schedule. A plain number still behaves exactly as before; a pattern just makes the count breathe."),
      code(
...
setCps(0.3)`,
  ...
  },
  {
    ...
    blocks: [
      ...
      p("`slices:` lets the NOTE pick a piece. `.chop(n)` does the reverse: it slices each event into n consecutive pieces of its OWN sample automatically, keeping the rhythm, so one `sample` hit becomes an in-order walk through the buffer with no note pattern to write. `s('break').chop(8)` in JavaScript, `chop 8` as a play modifier in rondo. The synth just plays the sound with `sample(gate, name)`; no `slices:` is needed, because each sliced event carries its own window. `.striate(n)` is the interleaved cousin: it plays the whole pattern n times, pass i taking slice i of every event, so a row of hits becomes n sweeps through the kit (the classic break shuffle) rather than one walk through a single hit."),
      p("The slice count can itself be a PATTERN, so it changes over time: `chop <4 8>` chops by four one cycle and eight the next. This is the same rule the rest of the language follows (see the note on patterned arguments below): anywhere a count goes, a `<…>` of counts goes too."),
      table(
