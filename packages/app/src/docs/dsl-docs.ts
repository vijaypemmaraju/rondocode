...
const MINI_SYNTAX: DocEntry[] = [
  ...
  ms('mini:|', 'a | b', 'Choice: each cycle picks one alternative at random, deterministically per cycle number, so a render repeats exactly. THE ALTERNATIVES ARE WHOLE SEQUENCES, which is the thing that surprises people: `5|1 4 3` chooses between the single note `5` and the three-note run `1 4 3`, not between `5` and `1` at the first step. Bracket it to choose one STEP: `[5|1] 4 3`. It does not mix with `,` at the same level -- `0|1, 2` is an error, because a stack of choices and a choice of stacks are both plausible readings and neither should be assumed. Say which: `[0|1], 2`.', "n('0 3 5 | 7 5 3')"),
  ms('mini:\'', "a'n  a'name:n", 'PER-NOTE LANES: values written ON the note, so they cannot drift when the notation grows a rest or a subgroup the way a parallel control line does. A bare number is the `expr` lane, read by the synth as param(\'expr\'). A named lane rides alongside it and they chain in any order. Three names are STRUCTURAL: `gain` is that note\u2019s level, `dur` a multiplier on its length, and `chance` the probability it sounds at all (reproducible, drawn from the same time-locked stream as degradeBy). Any other name is an ordinary param for that note alone. WRITE IT DIRECTLY AFTER THE NOTE, before any modifier: `0\'gain:.8@2` is right and `0@2\'gain:.8` is refused, because there the suffix attaches to the weight instead of the note.', "n(\"0'2 3'gain:.8 5'chance:.5\")"),
]
