...
const PATTERN_METHODS: DocEntry[] = [
  ...
  pm('slur', 'slur(prob?: number, seed?: number)', "SMART BOWING: derives per-note slide ties from the note content, the way a string player slurs. A note ties into the next only when the next starts exactly where it ends (a rest breaks the phrase) and carries a DIFFERENT pitch -- a slide tie between identical pitches erases the boundary entirely (no gate edge, no pitch change: the synth cannot know a new note happened), which is also why a real player re-articulates repeated notes even under a slur. prob (def 0.8) is the chance a tieable boundary ties, drawn deterministically from the boundary's exact time, so bow lengths vary but re-renders are bit-identical; mean slur length is about 1/(1-prob) notes. Explicit .slide() values win. Patterns loop, so a cycle's last note ties into the next cycle's first: end the phrase with a rest to breathe. Needs a mono synth to sound tied; pairs naturally with ddsp strings and winds.", ".slur(0.8)"),
...
const SYNTH_CTX: DocEntry[] = [
  ...
  sc(
    'ddsp',
    "ddsp(gate, model, opts?: { freq?, vel?, breath?, vib?, vibrate?, air?, bright?, scoop?, fall?, level?, dyn?, gain?, attack?, release?, punch?, vibdelay?, flow?, seed? })",
    "A TRAINED instrument: a small neural decoder (learned from real solo recordings -- violin, viola, cello, bass, flute, trumpet, tenorsax and piano ship free) turns pitch and loudness into 64 harmonic amplitudes plus a noise spectrum, re-decided ~94 times a second, and an additive renderer plays them. The consequence is that LOUDNESS CHANGES TIMBRE the way it does on the real instrument: a quiet violin is not a loud violin turned down, it loses bow noise and upper partials, and that is what the decoder learned. Pitch tracks the note and loudness follows velocity on their own. `breath` (dB, a signal) pushes the decoder's loudness input around while a note holds -- an lfo or envelope here is crescendo/decay INSIDE the note, the most expressive knob on the instrument. `vib` (semitones) and `vibrate` (Hz, def 5.5) run a delayed-onset vibrato like a real player's -- onset delay is `vibdelay` (s, def 0.6), and rate/depth drift a few percent on their own, because metronomic vibrato is the tell of a fake player. THE ARTICULATION SET (all signals): `air` rebalances the noise half against the harmonics (1 = as trained, 0 = pure tone, 2 = double breath/bow noise -- the two halves are separate until the final sum, so this is free); `bright` (-3..3) is a loudness-preserving spectral tilt, positive leaning on the upper harmonics like bow pressure; `scoop` (semitones) approaches each note from below over ~80 ms; `fall` (semitones) drops the pitch over ~120 ms at release. `punch` (dB, def 2.5) feeds the decoder extra loudness for the first ~80 ms of each note, so the MODEL plays the tongued/bowed accent it learned rather than an envelope faking one. `flow` (0..2, def 1) is NOTE-TO-NOTE conditioning: the decoder was trained on real performance curves, where pitch moves through a ~28 ms transition and loudness dips a few dB at every note change -- feeding it square steps is out-of-distribution and reads as notes pasted together. flow shapes the f0/loudness trajectories back into that distribution (plus the slow within-note settle every sustained instrument has), which is where the between-note sound comes from; 0 turns it off. Too short to hear as a slide -- this is not portamento. BOWING, in this vocabulary: bow pressure is `breath` (swell an env or lfo through it), bow noise is `air`, pressure brightening is `bright`, a bow change is the gate edge (punch plays the accent), and a SLUR is slide-tied notes on a mono voice -- the gate holds across the tie so nothing re-attacks, and the pitch SNAPS to the next note like a finger change (pinned by test). That is legato. Portamento is a different request: add glide (seconds) to the mono voice and tied notes smear between pitches instead. `level` (dB, def -15, where 0 dB is a full-scale sine) sets the loudness fed at full velocity, `dyn` (dB, def 30) how far velocity reaches down from it, and `gain` (linear, def 2) is output makeup -- the decoder reproduces the RECORDING's absolute level, which would sit far below an oscillator without it; `attack`/`release` (s, def 0.04/0.3) shape the loudness envelope. PIANO is a STRUCK model: it has no loudness input at all -- the decoder generates each note's decay from velocity and time since the strike, and its partials are stretched by a learned per-key inharmonicity table, so `breath`/`level`/`dyn` do nothing there; velocity is the dynamic, key-up engages the damper (`release` is the damper time), and `air`/`bright` still shape the tone. Models load lazily from the model CDN on first use (silence until the ~1 MB file lands, then it just starts sounding -- same live-load contract as samples). Self-enveloping: no ADSR needed. Mono, ~[-1,1]. Each instrument also ships a BODY impulse response (sample '<name>body', e.g. 'violinbody', loaded automatically with the model): the fixed body resonances the additive synth cannot make, learned from the recordings. Post-chain `input.mix(convolve(input, 'violinbody'), 0.5)` puts the wood back; add room reverb after it like a real player.",
