...
import { jsHeaderIndent, inJsBody } from '../jsblocks'
import { scanBalanced } from '@rondocode/rondo'
...
const rondoStreamLang = StreamLanguage.define<{ curve?: boolean; wgsl?: number; js?: number; jsNext?: number; jsInline?: boolean }>({
  ...
  startState: () => ({}),
  token(stream, state) {
    // A `visual` body is WGSL and a `js` body is JavaScript; wgsl.ts and
    // jsblocks.ts paint them as overlays. Emitting rondo tokens UNDER an
    // overlay would not change a colour (the marks nest innermost and win) but
    // it splits every one of them at every rondo token boundary — and worse,
    // wherever the overlay has no mark the rondo colour would show through,
    // painting escaped JS as music. So the tokenizer stands down and hands the
    // line over whole.
    if (stream.sol()) {
    ...
    // the braced span of an inline `js{ … }`: consumed whole (balanced, the
    // lexer's own scan) and left unstyled for the JS overlay to paint
    if (state.jsInline) {
      state.jsInline = false
      if (ch === '{') {
        const close = scanBalanced(stream.string, stream.pos)
        if (close >= 0) stream.pos = close + 1
    ...
    if (/[a-zA-Z_]/.test(ch)) {
      ...
      // `js` opens an escape hatch: a lone header starts the block form, and
      // `js` glued to a `{` starts the inline span (consumed above, next call)
      if (w === 'js') {
...
export const OPTIONS: RondoOption[] = [
  ...
  c('sing', 'keyword', 'sing NAME voice:barbara', 'A neural vocal: LYRIC lines above MELODY lines (pairs), one syllable per note. Modifiers below, then an optional `post` FX chain on the voice itself; the channel also takes `send`, `sidechain` and `out` like any synth.', 'sing vox voice:barbara\n  lo-ver come and sing with me\n  e4 e4 g4 g4 a4 g4 e4\n  gain: .95\n  post\n    reverb mix:.22'),
  ...
  c('stereo', 'keyword', 'stereo width:1.3 monobelow:120', 'Mid/side on the mix bus: scale the sides, and fold the low end to mono. Both are mono-safe -- the mono sum is untouched at any width.', 'stereo width:1.3 monobelow:120'),
  ...
  c('out', 'keyword', 'out SYNTH N..M', "Route a synth to hardware output channels of a multichannel interface, numbered like the jacks: `out click 3..4` sends the click to outputs 3/4 (a monitor feed, a stem to FOH). `1..2` is the master pair; a single number routes mono. Routed synths keep their strip, duck and sends but bypass the master stage; on a device without those outputs they fold back into the master pair.", 'out click 3..4'),
  ...
  c('mic', 'function', 'mic [device:NAME]', 'The LIVE microphone as a signal. Vocode it, filter it, granular it. device: names which input to open, and different synths may each name their own -- every named device gets its own live capture beside the default. After device: the editor completes the CONNECTED inputs (once mic permission has been granted); any part of the device label works. Headphones advised.'),
  ...
  c('pitchshift', 'function', 'pitchshift semitones:…', 'Shift pitch without changing length. `semitones:` is a SIGNAL, so a knob or a per-note lane can move the interval while it plays. `window:` trades smearing against warble; `mix:` keeps the dry underneath for a detuned double.', 'pitchshift semitones:iv mix:.5\niv = knob 7 -12..12'),
  ...
  c('follow', 'function', 'follow SIGNAL attack:… release:…', 'An envelope FOLLOWER: turns a signal into its own loudness, as a control signal. Feed it to a filter and the part opens itself. It is a processor, so in a binding it needs the signal to follow: `follow mic`, not `follow` alone.', 'amp = follow mic attack:8 release:160'),
  c('looper', 'function', 'looper REC feedback:… clear:…', 'A LOOP PEDAL on the running signal: REC high records -- the first press sets the loop length, later presses overdub layers onto it as it plays. The dry signal always passes; the loop stacks under it. feedback below 1 fades old layers per overdub pass; clear: wipes on a rising edge. name:jam gives the samples popover a bounce button that copies the loop into the bank as a sample.', 'looper rec feedback:.9\nrec = knob 0 0..1'),
  ...
  c('slur', 'keyword', 'slur [prob]', 'SMART BOWING: ties a note into the next only when it starts exactly where this one ends AND changes pitch -- rests breathe, repeats re-attack, like a real slur. prob (default .8) is the chance a tieable boundary ties, deterministic per boundary so re-renders match. Needs a mono synth to sound tied; made for the ddsp strings and winds.', 'play violin\n  0 2 4 5\n  slur .85'),
