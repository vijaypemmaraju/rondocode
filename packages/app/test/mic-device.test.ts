 ...
 *
 * JS ONLY for now. `mic device:scarlett` in rondo was written, and reverted:
 * giving `mic` a named argument changed how the parser binds a FOLLOWING
 * `name:value`, and the shipped live-mic example (`vocoder mic bands:24`)
 * started attributing `bands:` to `mic`. Binding a named arg to the nearest
 * call that ACCEPTS THAT NAME is a parser change, not a builtin one. */
